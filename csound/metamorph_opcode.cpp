...
struct Mm {
    ...
    TransientLPF* transient_lpf;  // ******
    TransientHPF* transient_hpf; // ******
...
Mm::Mm(CSOUND *csound, MM *params) { // Constructor
    ...
    transient_lpf = new TransientLPF(); // ******
    fx->add_transformation(transient_lpf);
    
    transient_hpf = new TransientHPF(); // ******
    fx->add_transformation(transient_hpf);    
}
...
extern "C" int mm(CSOUND *csound, MM *p) {
    ...
    p->data->transient_lpf->transientLPF((*p->transient_lpf)); // ******
    p->data->transient_hpf->transientHPF((*p->transient_hpf)); // ******
